...
Elm.Song = Elm.Song || {};
Elm.Song.make = function (_elm) {
   "use strict";
   _elm.Song = _elm.Song || {};
   if (_elm.Song.values) return _elm.Song.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([A2($Html$Events.onClick,address,NoOp),$Html$Attributes.$class("song")]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("avatar")]),_U.list([]))
              ,A2($Html.div,_U.list([$Html$Attributes.$class("name")]),_U.list([$Html.text(model.name)]))
              ,A2($Html.div,_U.list([$Html$Attributes.$class("metadata")]),_U.list([$Html.text(model.url)]))]));
   });
   var init = F2(function (url,name) {    return {url: url,name: name};});
   var Model = F2(function (a,b) {    return {url: a,name: b};});
   return _elm.Song.values = {_op: _op,init: init,update: update,view: view,Model: Model};
};
Elm.Visualizer = Elm.Visualizer || {};
Elm.Visualizer.make = function (_elm) {
   "use strict";
   _elm.Visualizer = _elm.Visualizer || {};
   if (_elm.Visualizer.values) return _elm.Visualizer.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var player = function (model) {
      ...
      if (_p0.ctor === "Just") {
            ...
            _U.list([$Html$Attributes.src(A2($String.append,_p0._0,"?autoplay=1"))
                    ,$Html$Attributes.width(390)
                    ,$Html$Attributes.height(200)
                    ,A2($Html$Attributes.property,"frameBorder",$Json$Encode.string("0"))]),
            _U.list([]));
         } else {
   ...
   var view = function (model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("visualizer")]),
      _U.list([A2($Html.div,_U.list([$Html$Attributes.$class("status")]),_U.list([$Html.text("Now Playing")])),player(model)]));
   };
   var update = F2(function (action,model) {    return model;});
   var NoOp = {ctor: "NoOp"};
   var init = function (url) {    return {url: url};};
   var Model = function (a) {    return {url: a};};
   return _elm.Visualizer.values = {_op: _op,init: init,update: update,view: view,Model: Model};
};
Elm.ELMusic = Elm.ELMusic || {};
Elm.ELMusic.make = function (_elm) {
   "use strict";
   _elm.ELMusic = _elm.ELMusic || {};
   if (_elm.ELMusic.values) return _elm.ELMusic.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $Song = Elm.Song.make(_elm),
   $Visualizer = Elm.Visualizer.make(_elm);
   var _op = {};
   var update = F2(function (action,model) {    var _p0 = action;return _U.update(model,{visualizer: $Visualizer.init($Maybe.Just(_p0._0))});});
   var SelectSong = function (a) {    return {ctor: "SelectSong",_0: a};};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.id("prototype")]),
      _U.list([$Visualizer.view(model.visualizer)
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("list")]),
              A2($List.map,
              function (song) {
                 return A2($Song.view,A2($Signal.forwardTo,address,$Basics.always(SelectSong(function (_) {    return _.url;}(song)))),song);
              },
              model.songs))]));
   });
   var init = {visualizer: $Visualizer.init($Maybe.Nothing)
              ,songs: _U.list([A2($Song.init,"https://www.youtube.com/embed/U6vp1EMnqho","Neil Young")
                              ,A2($Song.init,"https://www.youtube.com/embed/j9Dr1anRP9w","Concierto pa george")])};
   var Model = F2(function (a,b) {    return {visualizer: a,songs: b};});
   return _elm.ELMusic.values = {_op: _op,Model: Model,init: init,SelectSong: SelectSong,update: update,view: view};
};
Elm.App = Elm.App || {};
Elm.App.make = function (_elm) {
   ...
   var _op = {};
   var main = $StartApp$Simple.start({model: $ELMusic.init,update: $ELMusic.update,view: $ELMusic.view});
