...
Elm.ELMusic.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var view = F2(function (action,model) {    return A2($Html.div,_U.list([]),_U.list([$Html.text("Hola 7")]));});
   var update = F2(function (action,model) {    return model;});
   var NoOp = {ctor: "NoOp"};
   var init = {visualizer: 0,songs: _U.list([])};
   var Model = F2(function (a,b) {    return {visualizer: a,songs: b};});
   return _elm.ELMusic.values = {_op: _op,Model: Model,init: init,NoOp: NoOp,update: update,view: view};
};
Elm.App = Elm.App || {};
